...
patterns.reduce((acc, pattern) => acc.then(() => {
  ...
  shell.exec(`gnuplot -e "path='${commander.path}/'" ./plots/completeness-static.gnuplot`)
...
function average(pattern) {
  ...
  return new Promise((resolve, reject) => {
    ...
    files.reduce((acc, cur) => acc.then(() => {
      ...
      const average = []
      fs.writeFileSync(commander.path+'/'+pattern+'-global-completeness.csv', ["round","globalcompleteness","globalcompletenesscompleted","globalcompletenessincresults","messages","completeQueries","queriesNumber","obtainedresults","wantedresults","timedout"].join(',')+'\n', 'utf8')
      for(let i =0; i<results[0].length; ++i) {
        ...
        for(let j = 0; j<results.length;++j) {
          ...
          //console.log(Object.values(val), Object.values(val).length)
